...
<div class="userpay-view">
  ...
  <br>
    ...
    <?php 
        ...
        <thead><tr><th>Id</th><th>Uid</th><th>Phone</th><th>Date</th><th>Type</th><th>From</th><th>Amount(AGP)</th><th>Amount(agp)</th><th>Status</th><th  class="operation">Operation</th></tr></thead>';

        $number = $model->phone; // user phone
        $fnumber = Yii::$app->db->createCommand("SELECT * FROM `transaction` WHERE `number` ='$number'")->queryAll();

        
        foreach (array_reverse($fnumber) as $n) {echo "<tr><td>".$n['id']."</td><td>".$n['uid']."</td><td>".$n['number']."</td><td>".$n['date']."</td><td>".$n['action']."</td><td>".$n['typet']."</td><td>".$n['amount']."</td><td>".$n['balagp']."</td><td>".$n['statust'].'</td><td class="operation">'; if($n['statust']!='canceled') echo '<a href="#" onclick = "cancelid('.$n['id'].',\''.$n['action'].'\','.$n['balagp'].','.$n['uid'].')"  >Cancel</a>'; echo '</td></tr>';
        }
        echo '</table></div>';
    ?>
